...
4. **Site-wide facts** (name, address, email, social links, the resume path, the availability line) come from `src/lib/site.ts`. Project words come from one YAML file per project in `src/content/projects/`, checked by `scripts/content-build.mjs` and read through `src/content/projects.ts` ([DECISIONS.md](../DECISIONS.md) entries 21 and 22). Never edit `projects.generated.json` and never put project words back into TypeScript. Status is worked out from `live` and `code`; never add a status field. The file format is in [guides/case-studies.md](../guides/case-studies.md). Words that belong to a section stay in that section's component; words that appear in more than one place get one home in `site.ts`.

## The stage

4a. **Only the owner publishes.** An agent may draft `facts` and a `case_study` from the owner's answers and the code, but a case study stays `published: false` until the owner has read it, and facts are only added once he has approved them. Never add placeholders to make a tile look full ([DECISIONS.md](../DECISIONS.md) entry 22).
...
23a. **Case study tests run once per published case study** and skip themselves while there are none. To check the case study page before any is published, publish a test case study in your own workspace, run the tests, and remove it before handover; never hand over a placeholder.
