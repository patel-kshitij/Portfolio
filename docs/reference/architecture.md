...
- **Architecture drawings.** `ArchitectureDiagram` places boxes on a grid of up to five columns and two rows. Arrows along a row join the boxes' sides; arrows between rows leave from the bottom or top. Two arrows between the same boxes, one each way, are drawn as two parallel lines with their step badges on either side ([DECISIONS.md](../DECISIONS.md) entry 26). Colours come from `Architecture.module.scss`. The full drawing never shrinks below 30rem; its frame scrolls sideways on a phone. Its accessible name lists every arrow in words ("Finder phone to Tag page, ..."), and the small tile's version is hidden from screen readers because the tile's button already names the project.
